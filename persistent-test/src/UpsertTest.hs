...
specsWith runDb handleNull handleKey = describe "UpsertTests" $ do
    let ifKeyIsPreserved expectation =
            case handleKey of
    ...
    describe "upsertBy" $ do
        ...
        it "does not require all unique keys to match" $ runDb $ do
            let initial = UpsertBy "a" "b" "c"
            _ <- insert initial

            let modified = UpsertBy "d" "e" "f"
            Entity _ upserted <-
                upsertBy (UniqueUpsertBy "a") modified [UpsertByAttr =. "blargh"]
            upserted @== initial { upsertByAttr = "blargh" }
